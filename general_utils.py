...
def flatten_json(file, proc_func, loc):
    with open(file, encoding="utf8") as f:
        data = json.load(f)['data']
    rows, contexts, nodes, edges  = [], [], [], []
    for i in range(len(data)):
        print(i)
        with open(join('/home/wanglihan/DP_{}'.format(loc),'{0}_{1}_dp.json'.format(i ,loc))) as v:
            graph=json.load(v) #dict
        partial_rows, context, partial_node, partial_edge = proc_func(i, data[i], graph)
        rows.extend(partial_rows)
        contexts.append(context)
        nodes.append(partial_node)
        edges.append(partial_edge)
    return rows, contexts, nodes, edges
...
def feature_gen(C_docs, Q_CID, Q_docs, no_match):
    C_tags = [[w.tag_ for w in doc] for doc in C_docs] #tag、tag_：token的词性（Fine-grained POS）
    C_ents = [[w.ent_type_ for w in doc] for doc in C_docs] #ent_type、ent_type_：命名实体类型
    C_features = []

    for question, context_id in zip(Q_docs, Q_CID):
        context = C_docs[context_id]

        counter_ = Counter(w.text.lower() for w in context) #counter计数软件
        ...
        if no_match:
            ...
            question_lemma = {w.lemma_ if w.lemma_ != '-PRON-' else w.text.lower() for w in question} #lemma、lemma_：token的基本形式（base form）
...
class BatchGen_QuAC:
    ...
    def __iter__(self):
        ...
        idx_perm = range(0, self.context_num)
        if not self.eval:
            idx_perm = np.random.permutation(idx_perm)  #对一个list进行随机排序

        batch_size = self.batch_size
        for batch_i in range((self.context_num + self.batch_size - 1) // self.batch_size): #" / "就表示 浮点数除法，返回浮点结果;" // "表示整数除法。
            #idx_perm打乱的数组
            batch_idx = idx_perm[self.batch_size * batch_i: self.batch_size * (batch_i+1)]
            ...
            context_batch = list(zip(*context_batch))  #context和context合在一起 node和node合在一起
            ...
            # wait_to_node=context_batch[7]
            # wait_to_edge=context_batch[8]
            max_node_num, max_node_length=0, 0
            node_batch = []
            for context_node_arr in context_batch[7]:
                #context_node_arr包含一个context里的结点集合的arr
                max_node_num=max(max_node_num,len(context_node_arr))
                for node_word_arr in context_node_arr:
                    max_node_length= max(max_node_length, len(node_word_arr))
            node_id=torch.LongTensor(batch_size, max_node_num, max_node_length ).fill_(0)

            for c_n_a_index, c_n_a in enumerate(context_batch[7]):
                for j, c_n_inside in enumerate(c_n_a):
                    node_id[c_n_a_index, j, :len(c_n_inside)]=torch.LongTensor(c_n_inside)
                for j in range(len(c_n_a), max_node_num):
                    node_id[c_n_a_index, j, :2]=torch.LongTensor([4, 5])

            #Process Edge
            #edge_id (batch, max_node_num, max_node_num)
            edge_id=torch.LongTensor(batch_size, max_node_num, max_node_num ).fill_(0)
            # print('maxnode_num{}'.format(max_node_num))
            # print(len(context_batch[8]))
            # print(max([len(x) for x in context_batch[8]]))
            for bat_idx, e_n_a in enumerate(context_batch[8]):
                for e_j, e_n_inside in enumerate(e_n_a):
                    edge_id[bat_idx, e_j, :len(e_n_inside)]=torch.LongTensor(e_n_inside)
                # for e_j in range(len(e_n_a),max_node_num):
                #     edge_id[bat_idx, e_j, :2] = torch.LongTensor([4, 5])
            ...
            for first_QID in context_batch[5]:
                ...
                while True:
                    if first_QID + i >= len(qa_data) or qa_data[first_QID + i][0] != qa_data[first_QID][0]: # their corresponding context ID is different
                        break
                    question_seq.append(first_QID + i) #[4,5,6]
                    question_len = max(question_len, len(qa_data[first_QID + i][1]))
                    i += 1
                question_batch.append(question_seq) #[[11,12,13,14],[15,16,17]]
                question_num = max(question_num, i)  #10 每个context问题个数
            ...
            for i, q_seq in enumerate(question_batch):
                ...
                for j in range(len(q_seq), question_num):  #start: len(q_seq)  end: question_num #针对空的question
                    question_id[i, j, :2] = torch.LongTensor([2, 3])
                    question_tokens.append(["<S>", "</S>"])
            ...
            # Process Masks
            context_mask = torch.eq(context_id, 0).bool() #包含了每个位置的比较结果(相等为1，不等为0)
            question_mask = torch.eq(question_id, 0).bool()
            node__mask=torch.eq(node_id, 0).bool()
            ...
            if self.gpu: # page locked memory for async data transfer
                ...
                context_cid = context_cid.pin_memory()
                question_cid = question_cid.pin_memory()
                node_id = node_id.pin_memory()
                edge_id = edge_id.pin_memory()
            #cid貌似是allennlp elmo的id
            yield (context_id, context_cid, context_feature, context_tag, context_ent, context_mask,
                   question_id, question_cid, question_mask, overall_mask,
                   answer_s, answer_e, answer_c,
                   text, span, question, answer, node_id, node__mask, edge_id)
