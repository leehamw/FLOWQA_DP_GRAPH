...
class FlowQA(nn.Module):
    ...
    def __init__(self, opt, embedding=None, padding_idx=0):
        ...
        #graph
        self.graph_encoder=layers.GraphEncoder(embedding_dim, opt['hidden_size'] )
    ...
    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask, node_id, node_mask, edge_id ):
        """Inputs:
        ...
        node=node_id.view(-1, node_id.size(-1)).contiguous() #(batch*max_node_num), max_node_length
        # print('node{}'.format(node))
        node_full_mask=node_mask.view(-1, node_mask.size(-1))  ##(batch*max_node_num), max_node_length
        ...
        x2_mask = x2_full_mask.view(-1, x2_full.size(-1))  #（(batch*q_num),len_q)
        # print('embeddingweight{}'.format(self.embedding.weight))
        ...
        if self.opt['use_wemb']:
            ...
            node_emb=emb(node) #(batch*max_node_num), max_node_length, emb_size
            ...
            if self.opt['dropout_emb'] > 0:
                ...
                node_emb = layers.dropout(node_emb, p=self.opt['dropout_emb'], training=self.training)
        ...
        node_input=torch.cat(grnn_input_list, dim=2)  #(batch*max_node_num), max_node_length, emb_size
        ...
        x1_emb_expand = expansion_for_doc(x1_emb)  #(batch*q_num),len_d,emb_size
        ...
        pre_node_emb=node_emb.view(node_id.size(0), node_id.size(1), node_emb.size(1), node_emb.size(2)).contiguous()
        pre_node_emb_expand=pre_node_emb.unsqueeze(1).expand(pre_node_emb.size(0), x2_full.size(1), pre_node_emb.size(1), pre_node_emb.size(2), pre_node_emb.size(3)).contiguous()
        #(batch*q_num), max_node_num, max_node_length, emb_size
        #要用的
        node_emb_expand=pre_node_emb_expand.view(-1,pre_node_emb_expand.size(2), pre_node_emb_expand.size(3), pre_node_emb_expand.size(4))

        #node_mask [batch * max_node_num * max_node_length]->[batch * q_num * max_node_num * max_node_length]
        pre_node_emb_expand_mask=node_mask.unsqueeze(1).expand(node_mask.size(0), x2_full.size(1), node_mask.size(1), node_mask.size(2)).contiguous()
        # #(batch*q_num),  max_node_num, max_node_length
        #要用的
        node_emb_expand_mask=pre_node_emb_expand_mask.view(-1,node_mask.size(1), node_mask.size(2))

        #edge_id     [batch * max_node_num * max_node_num ]  [batch *q_num * max_node_num * max_node_num ]
        pre_edge_expand=edge_id.unsqueeze(1).expand(edge_id.size(0), x2_full.size(1), edge_id.size(1),  edge_id.size(2)).contiguous()
        edge_expand=pre_edge_expand.view(-1, edge_id.size(1),  edge_id.size(2))
        ...
        x1_mask = x1_full_mask.view(-1, x1_full_mask.size(-1)) # (batch*q_num, len_d)

        if self.opt['do_prealign']:
            x1_atten = self.pre_align(x1_emb_expand, x2_emb, x2_mask) # # batch*q_num* lend * xq_input_size
        ...
        doc_hiddens = self.doc_rnn1(x1_input, x1_mask) # (batch*q_num, len_d, hidden_size)
        graph_output = self.graph_encoder(doc_hiddens, x1_mask, node_emb_expand, node_emb_expand_mask,
                                          edge_expand)  # bsz', max_node_num,  hidden
        # doc_hiddens=graph_output
        doc_hiddens_flow = flow_operation(doc_hiddens, self.dialog_flow1) # [bsz * q_num, len_d, flow_hidden_state_dim]



        doc_abstr_ls.append(graph_output)

        doc_hiddens = self.doc_rnn2(torch.cat((doc_hiddens, doc_hiddens_flow, x1_cove_high_expand), dim=2), x1_mask) #opt['hidden_size'] * 2 + flow_size + CoVe_size
        doc_hiddens_flow = flow_operation(doc_hiddens, self.dialog_flow2) # [bsz * q_num, len_d, flow_hidden_state_dim]
        ...
        _, que_abstr_ls = self.question_rnn(x2_input, x2_mask, return_list=True, additional_x=x2_cove_high) # [((batch*q_num), len_q, hidden_size)]
        ...
        [torch.cat([x2_emb, x2_cove_high], 2)], que_abstr_ls, x1_mask, x2_mask) # # batch*q_num * len1 * x2_input_size
        ...
        doc_abstr_ls += [doc_hiddens] #[(batch*q_num, len_d, hidden_size),(batch*q_num, len_d, hidden_size),(batch*q_num, len_d, hidden_size)]
        ...
        if self.opt['self_attention_opt'] > 0:
            ...
            doc_hiddens = self.high_lvl_crnn(torch.cat([doc_hiddens, doc_hiddens_flow], dim=2), x1_mask) # (batch*q_num, seq_len, hidden_size)

        doc_abstr_ls += [doc_hiddens]

        # Merge the question hidden vectors
        q_merge_weights = self.self_attn(question_hiddens, x2_mask)
        question_avg_hidden = layers.weighted_avg(question_hiddens, q_merge_weights) #(batch*q_num )* hidden
        if self.opt['do_hierarchical_query']:
           ...
           #question_avg_hidden  (bsz, q_num, hidden)
            ...
            question_avg_hidden = question_avg_hidden.contiguous().view(-1, question_avg_hidden.size(-1)) # (batch*q_num ), hidden
        ...
        doc_avg_hidden = torch.cat((torch.max(doc_hiddens, dim=1)[0], torch.mean(doc_hiddens, dim=1)), dim=1) #batch, hidden
