...
class QAModel(object):
    ...
    def update(self, batch):
        ...
        if self.opt['cuda']:
            ...
            for e1 in batch[-3:]:
                inputs.append(e1.cuda(non_blocking=True))
            ...
            for e in batch[-3:]:
                inputs.append(e.cuda(non_blocking=True))
        ...
        for i in range(overall_mask.size(0)):
            ...
            if self.opt['question_normalize']:
                #损失函数名字中带了with_logits. 而这里的logits指的是,该损失函数已经内部自带了计算logit的操作
                ...
                single_loss = single_loss + F.cross_entropy(score_s[i, :q_num], target_s) * (q_num - sum(target_no_answ.float())).item() / 7.0
                single_loss = single_loss + F.cross_entropy(score_e[i, :q_num], target_e) * (q_num - sum(target_no_answ.float())).item() / 7.0
            else:
            ...
            # print('single_loss{}'.format(single_loss))
